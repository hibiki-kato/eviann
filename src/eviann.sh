...
if [ ! -e protein2genome.align.success ];then
  if [ -s $PROTEINFILE ];then
    ...
    (set -o pipefail; miniprot -p 0.95 -N 20 -k $KMERVALUE -t $NUM_THREADS -G $MAX_INTRON --gff $GENOMEFILE $PROTEIN.uniq 2>miniprot.err | \
...
#this produces files $GENOME.{k,u}.gff.tmp  and $GENOME.unused_proteins.gff.tmp
  ...
  if [ $NUM_TISSUES -gt 0 ];then
    log "Computing Markov chain matrices at splice junctions, negative training data" && \
...
#we now filter the transcripts file using the splice scores, leaving alone the transcripts that do match proteins and rerun combine
  ...
  if [ $(wc -l $GENOME.spliceFiltered.gtf|awk '{print $1}') -eq 0 ];then 
    error_exit "Transcript file is empty, likely insufficient RNA-seq data, exiting...";
  fi && \
...
#these are u's -- no match to a protein, use transdecoder to try to find CDS
  if [ -s $GENOME.u.gff ];then
    ...
    TransDecoder.LongOrfs -m $MIN_ORF -S -t $GENOME.lncRNA.fa 1>transdecoder.LongOrfs.out 2>&1 && \
...
#here if we have external CDSs and proteins we figure out which ones do not overlap with annotated genes or best unused proteins, and add them as EXTERNAL
  if [ -s $CDSFILE ] && [ -s $GENOME.$PROTEIN.uniq.palign.gff ];then
    ...
    if [ -s $EXTERNAL_CDS_GFF ];then
      gffcompare -T -r <(cat $GENOME.k.gff $GENOME.best_unused_proteins.gff) -o externalk $EXTERNAL_CDS_GFF && \
      #we do not want to add externals that match transcripts to the transcript file (best unused proteins)
      gffcompare -T -r $GENOME.abundanceFiltered.spliceFiltered.gtf -o externalt $EXTERNAL_CDS_GFF && \
      gffread -F --keep-exon-attrs --ids <(perl -F'\t' -ane '{if($F[8]=~/transcript_id "(\S+)";.+class_code "(u|p|o|x|s)";/){print "$1\n"}}' externalk.annotated.gtf ) $EXTERNAL_CDS_GFF | \
      ...
      gffcompare -T -r $EXTERNAL_CDS_GFF $GENOME.u.cds.gff -o externalu && \
...
#the file $GENOME.palign.all.gff contains all CDSs we need to use
  ...
  TransDecoder.LongOrfs -m $MIN_ORF -S -t $GENOME.broken_cds.fa 1>transdecoder.LongOrfs.out 2>&1 && \
...
#we have everything, this is final combine
  cat $GENOME.palign.all.gff | \
    combine_gene_protein_gff.pl \
      --prefix $GENOME \
